...
serve(async (req) => {
  ...
  try {
    ...
    if (String((ag as any).status) === "cancelado") return json(req, { ok: true, status: "cancelado" });

    const { error: updErr } = await sb.from("agendamentos").update({ status: "cancelado" }).eq("id", agendamento_id);
    if (updErr) throw updErr;

    return json(req, { ok: true, status: "cancelado" });
  } catch (e: any) {
