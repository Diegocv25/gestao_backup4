...
serve(async (req) => {
  ...
  try {
    const { token, servico_id } = (await req.json()) as { token?: string; servico_id?: string };
    ...
    const { data, error } = await sb
      ...
      .select("funcionario_id, funcionarios:funcionarios(id,nome,ativo,carga)")
      .eq("servico_id", servico_id);
    if (error) throw error;

    const items = (data ?? [])
      .map((r: any) => r.funcionarios)
      .filter((f: any) => f && f.ativo === true && String(f.carga ?? "profissional") === "profissional")
      .map((f: any) => ({ id: String(f.id), nome: String(f.nome) }))
      .sort((a: any, b: any) => a.nome.localeCompare(b.nome));

    return json(req, { ok: true, profissionais: items });
  } catch (e: any) {
