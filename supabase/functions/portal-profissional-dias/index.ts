...
serve(async (req) => {
  ...
  try {
    ...
    // Confirma que o profissional pertence ao salão do token
    const { data: func, error: funcErr } = await sb
      .from("funcionarios")
      .select("id")
    ...
    if (!func) return json(req, { ok: false, error: "profissional não encontrado" }, { status: 404 });
    ...
    const dias = Array.from(
      new Set(
        (rows ?? [])
          .filter((r: any) => !!r && !!r.inicio && !!r.fim)
          .map((r: any) => Number(r.dia_semana))
          .filter((n) => Number.isFinite(n)),
      ),
    ).sort((a, b) => a - b);

    return json(req, { ok: true, dias });
  } catch (e: any) {
